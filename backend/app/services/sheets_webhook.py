...
def order_to_sheet_row(order) -> dict:
    return {
        ...
        "state": getattr(order, "governorate", None) or "",
        "city": getattr(order, "area", None) or "",
        "zip": getattr(order, "block", None) or "",
        "address1": getattr(order, "street", None) or "",
        "address2": getattr(order, "building", None) or "",
        ...
        "status": order.status,
        "payment_status": getattr(order, "payment_status", None) or "",
        "stripe_session_id": getattr(order, "stripe_session_id", None) or "",
    }
